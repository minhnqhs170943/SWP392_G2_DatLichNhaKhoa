...
const getAllServicesList = async () => {
    try {
        ...
        // Bỏ "WHERE IsActive = 1" để tránh lỗi không tồn tại cột
...
const getPendingAppointments = async (userId, search, startDate, endDate, service, page, limit) => {
    ...
    if (search) {
        conditions += ` AND u.FullName LIKE @Search`;
    ...
    // Thay STRING_AGG bằng SELECT TOP 1 để tránh lỗi tương thích SQL Server phiên bản cũ
    const query = `
        SELECT 
            ...
            ISNULL(a.Note, N'Không có ghi chú') AS note,
            (
...
const updateAppointmentStatus = async (appointmentId, status, note, userId) => {
    const pool = await poolPromise;
    const request = pool.request();
